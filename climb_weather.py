...
def get_geo_info() -> tuple:
    ...
    try:
        ...
        return geo_data["latitude"], geo_data["longitude"], geo_data["region_code"]
...
def get_rc_spots() -> list:
    ...
    try:
        ...
        print("Error! Did not get the valid Geocode.")
...
def get_forecast() -> None:
    ...
    while len(plan) < 5 and near_spots:
        distance, area_ID, area_name = heapq.heappop(near_spots)
                ...
                if day["temperatureHigh"] <= 82 and day["temperatureLow"] >= 65:
                    if float(day['precipProbability']) <= 0.1:
                        if day["windSpeed"] <= 6:
                            ...
                            plan.append((date, distance, area_name))
    # print(for_check)
    if len(plan) > 0:
        for spot in plan:
            print(f"{spot[2]} (distance: {spot[1]} miles) is good on {spot[0].strftime('%b-%d (%a)')}")
    else:
